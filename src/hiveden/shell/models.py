...
class ShellOutput(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of output"
    )
...
class ShellSessionCreate(BaseModel):
    ...
    docker_command: Optional[str] = Field(
        "/bin/bash", description="Shell command for Docker exec"
    )
...
class PackageCheckRequest(BaseModel):
    ...
    package_manager: Optional[str] = Field(
        "auto", description="Package manager (apt, yum, dnf, pacman, or auto)"
    )
...
class PackageInstallRequest(BaseModel):
    ...
    yes_to_prompts: bool = Field(
        True, description="Automatically answer yes to prompts"
    )
